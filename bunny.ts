...
export class Bunny {
    ...
    #err?: ErrorDef;
    #stache?: Stache;
    #tmplMeta = new WeakMap<Function, string>();

    #add(method: string, pattern: string, handler: Function, template?: string) {
        this.#routes.push({
            method,
            pattern,
    ...
    #dec(method: string, pattern: string) {
        const self = this;
        const reg: any = (...a: any[]) => {
    ...
    #detect(args: any[], cb: (h: Function) => void) {
        let h: Function | undefined;
    ...
    #register(method: string, pattern: string, ...rest: any[]) {
        if (rest.length === 0) return this.#dec(method, pattern);
    ...
    route(prefix: string, sub: Bunny) {
        ...
        if (sub.#err && !this.#err) this.#err = sub.#err;
    }
    ...
    async #onError(e: any, ctx: Context): Promise<Response> {
        if (!this.#err) {
            return new Response(e.message || "Internal Server Error", { status: 500 });
        }
        const result = await this.#err.handler(e, ctx);
        const status = e instanceof HttpError ? e.status : 500;
        if (this.#err.template && this.#stache) {
            const html = await this.#stache.view(this.#err.template, result);
...
function toResponse(val: any, status?: number): Response {
    if (val instanceof Response) return status ? new Response(val.body, { status, headers: val.headers }) : val;
    if (val == null) return new Response(null, { status: 204 });
    if (typeof val === "string") return new Response(val, { headers: { "Content-Type": "text/html; charset=utf-8" } });
    if (typeof val === "object") return Response.json(val, status ? { status } : undefined);
