...
routes.get("/", async (c: Context) => "Hello from Bunny!");
routes.get("/json", async (c: Context) => ({ message: "Hello from Bunny!", timestamp: Date.now() }));
...
routes.get("/headers", async (c: Context) => Object.fromEntries(c.req.headers));
...
routes.delete("/data/:id", async (c: Context) => c.json({ id: +c.params.id, deleted: true }));
